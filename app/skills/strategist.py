...
Write `summary` as three or four plain sentences a person would find useful
months later: what this account is about, who watches it, how the posts look and
sound, and what is obviously working. No bullet points, no headings."""
...
READ_SCHEMA = {
    ...
    "required": ["handle", "platform", "niche", "audience", "language", "summary"],
}
...
2. OFFER IDEAS when asked for them, or when the person names a channel and a
   ...
   person can tap. Each idea must be specific enough to film: "Three Uzbek foods
   foreigners always get wrong" is an idea, "food content" is not. Ground them in
   what you know about the channel: its niche, its audience, its language, what
   already works there. Six ideas at most.
    ...
    "properties": {
        ...
        "ideas": {
            ...
            "items": {
                ...
                "required": ["title", "hook", "why", "shape", "seconds", "topic"],
            },
        },
        "asks": {
            "type": "array",
...
async def read_profile(data: bytes, mime: str, platform: str = "") -> dict[str, Any]:
    ...
    try:
        ...
        return {"handle": "", "platform": platform or "other", "niche": "",
                "audience": "", "language": "", "summary": ""}
    if not isinstance(out, dict):
        return {"handle": "", "platform": platform or "other", "summary": ""}
    if platform:
...
def describe_profiles(profiles: list[dict[str, Any]]) -> str:
    ...
    again with every message would pay for it on every turn of the conversation
    and tell the model nothing it was not already told.
    """
    if not profiles:
        return "The person has not shown you any of their channels yet."
    lines = []
    for p in profiles:
        name = p.get("handle") or "(nomsiz)"
        lines.append(f"- {PLATFORMS.get(p.get('platform'), p.get('platform') or '?')} "
                     f"{name}: {p.get('summary') or 'hech narsa o‘qilmadi'}")
    return "The person's own channels:\n" + "\n".join(lines)
